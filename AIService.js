...
26. YOU MUST naturally weave in the related topics provided above throughout the content
27. Dedicate at least 1-2 paragraphs in each H2 section to connecting related topics
28. Use related topics to create deeper, more comprehensive coverage of the main topic  
29. Explain how related topics interconnect with and enhance the main topic
30. Guide readers to see broader patterns and connections through related topics

🚀 ADVANCED SEO OPTIMIZATION:
31. Include target keyword and variations naturally throughout the content (1-2% density)
...
- 6000-8000 characters of high-quality content (not too long)
- Maximum 5-6 main sections (H2 headings)
...
/**
 * OpenAI GPT로 언어별 글 생성
 */
function generateWithOpenAILanguage(topic, apiKey, model = "gpt-4o-mini", targetLanguage = "EN") {
  Logger.log(`=== OpenAI API 언어별 호출 시작 ===`);
  Logger.log(`모델: ${model}`);
  Logger.log(`토픽: ${topic}`);
  Logger.log(`타겟 언어: ${targetLanguage}`);
  
  const profile = getModelProfile(model);
  const prompt = buildModelOptimizedPromptWithLanguage(topic, profile, targetLanguage);
 ...
 * 언어별 모델 최적화 프롬프트 생성
 */
function buildModelOptimizedPromptWithLanguage(topic, modelProfile, targetLanguage = "EN") {
  const basePrompt = buildStructuredPromptWithLanguage(topic, targetLanguage);
  
  switch (modelProfile.strategy.promptTemplate) {
    case 'simple':
      if (targetLanguage === "KO") {
        return `다음 주제에 대해 한국어로 포괄적인 블로그 글을 작성해주세요: ${topic}

중요한 지시사항:
1. 한국 독자를 위한 자연스러운 한국어로 작성
2. 친근하고 유익한 내용으로 구성
3. 적절한 HTML 형식과 제목, 문단 사용

다음 JSON 형식으로 응답해주세요:
{
  "title": "SEO 최적화된 한국어 제목",
  "seoDescription": "155자 이내의 메타 설명",
  "categories": ["카테고리1", "카테고리2"],
  "tags": ["태그1", "태그2", "태그3"],
  "html": "적절한 구조의 완전한 HTML 내용"
}

주제: ${topic}
기억하세요: 모든 내용을 한국어로 작성해주세요.`;
      } else {
        return `Write a comprehensive blog post about: ${topic}
...
Remember: Write everything in English, even if the topic is in Korean.`;
    ...
    case 'gpt5_optimized':
      if (targetLanguage === "KO") {
        return `🤖 GPT-5 한국어 전문 프롬프트 🤖

당신은 GPT-5로서 한국어 콘텐츠 제작에 특화된 최신 AI 모델입니다. 향상된 능력을 활용하여 뛰어난 한국어 블로그 글을 작성해주세요.

미션: 다음 주제에 대한 포괄적이고 매력적인 한국어 블로그 글 작성: ${topic}

🎯 GPT-5 한국어 향상된 지시사항:
1. 언어: 자연스럽고 유창한 한국어로 전체 작성
2. 품질: 향상된 추론 능력으로 우수한 콘텐츠 제작
3. 구조: 체계적이고 읽기 쉬운 내용 구성
4. 참여도: 인간 심리에 대한 향상된 이해력 활용

📝 콘텐츠 사양:
- 2500자 이상의 고품질 내용
- 친근하면서도 권위 있는 톤
- 실행 가능한 인사이트와 실용적 가치
- 의미적 태그를 사용한 적절한 HTML 구조
...
📋 정확한 JSON 출력 필요:
{
  "title": "매력적인 한국어 제목 (50-60자)",
  "seoDescription": "매력적인 메타 설명 (150-155자)",
  "categories": ["주요카테고리", "부카테고리"],
  "tags": ["키워드1", "키워드2", "키워드3", "키워드4", "키워드5"],
  "html": "<h2>, <h3>, <p> 구조의 완전한 HTML"
}

⚡ GPT-5 한국어 콘텐츠 규칙:
- 고급 어휘를 적절히 사용
- GPT-5만이 제공할 수 있는 독특한 관점 포함
- GPT-4 출력과 차별화되는 콘텐츠 작성
- 완벽한 JSON 형식 보장
- 전체 내용을 한국어로 작성
...
/**
 * OpenAI GPT로 글 생성 (프로파일 기반) - 하위 호환성
 */
...
/**
 * Google Gemini로 언어별 글 생성
 */
function generateWithGeminiLanguage(topic, apiKey, model = "gemini-1.5-flash", targetLanguage = "EN") {
  const prompt = buildStructuredPromptWithLanguage(topic, targetLanguage);
...
/**
 * Anthropic Claude로 언어별 글 생성
 */
function generateWithClaudeLanguage(topic, apiKey, model = "claude-3-5-haiku-20241022", targetLanguage = "EN") {
  const prompt = buildStructuredPromptWithLanguage(topic, targetLanguage);
 ...
 * xAI Grok으로 언어별 글 생성
 */
function generateWithGrokLanguage(topic, apiKey, model = "grok-beta", targetLanguage = "EN") {
  const prompt = buildStructuredPromptWithLanguage(topic, targetLanguage);
...
function generateHtmlWithLanguage(topic, targetLanguage = "EN", relatedTopics = []) {
  ...
  Logger.log(`주제: ${topic}`);
  Logger.log(`타겟 언어: ${targetLanguage}`);
  Logger.log(`관련 주제: ${relatedTopics ? relatedTopics.length : 0}개`);
  if (relatedTopics && relatedTopics.length > 0) {
    Logger.log(`- 관련 주제 목록: ${relatedTopics.slice(0, 5).join(', ')}${relatedTopics.length > 5 ? '...' : ''}`);
  }
  ...
  for (let attempt = 1; attempt <= modelProfile.strategy.retryAttempts; attempt++) {
    try {
      ...
      switch (config.AI_PROVIDER) {
        case 'openai':
          result = generateWithOpenAILanguage(topic, config.AI_API_KEY, config.AI_MODEL, targetLanguage, relatedTopics);
          break;
          
        case 'gemini':
          result = generateWithGeminiLanguage(topic, config.AI_API_KEY, config.AI_MODEL, targetLanguage, relatedTopics);
          break;
          
        case 'anthropic':
          result = generateWithClaudeLanguage(topic, config.AI_API_KEY, config.AI_MODEL, targetLanguage, relatedTopics);
          break;
          
        case 'xai':
          result = generateWithGrokLanguage(topic, config.AI_API_KEY, config.AI_MODEL, targetLanguage, relatedTopics);
          break;
